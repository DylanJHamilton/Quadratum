...
(async () => {
  ...
  assert.equal(doc.querySelector('.qtm404__button--primary').getAttribute('href'), '/fr/'); assert.equal(doc.querySelector('.qtm404__button--secondary').getAttribute('href'), '/fr/collections'); assert.equal(doc.querySelector('form').getAttribute('action'), '/fr/search'); assert.equal(doc.querySelector('form').method, 'get'); assert.equal(doc.querySelector('input[type="search"]').name, 'q'); assert.equal(doc.querySelector('label').htmlFor, doc.querySelector('input[type="search"]').id); assert.equal(doc.querySelector('input[type="search"]').placeholder, 'Try "this"'); assert.equal(doc.querySelectorAll('.qtm404__link').length, 1); assert.equal(doc.querySelector('.qtm404__link').textContent.trim(), 'Helpful <safe>'); assert.equal(doc.querySelector('[data-editor-block]').getAttribute('data-editor-block'), '0'); assert.equal(doc.querySelector('.qtm404').style.getPropertyValue('--qtm404-padding-top').trim(), '0px'); close(d);
