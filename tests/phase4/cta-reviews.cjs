...
const blocks=['a','b'].map(id=>({id,type:'review',settings:{...defaults(schema.blocks[0].settings),reviewer_image:{id:'reviewer'},reviewer_location:'Location',verified_purchase:true,reviewer_name:'Name "quoted"',review_text:'Text </script><img src=x>',review_video_url:'/'+id+'.mp4',product:{title:'Product "quoted"',url:'/products/example'},review_image:{id},rating:4}}));
...
(async()=>{
 for(const mode of ['slideshow','grid','masonry','carousel']){
  ...
  assert(doc.getElementById('literal-anchor'));assert.equal(doc.querySelectorAll('video[controls]').length,2,'review video in every layout');assert.equal(doc.querySelectorAll('.q-reviewer-location').length,2);assert.equal(doc.querySelectorAll('.q-verified-badge').length,2);assert(!doc.querySelector('a[href="#"]'));assert.equal(doc.querySelectorAll('img[src=x]').length,0,'review text cannot become markup');assert(doc.querySelector('.q-description')===null);assert(!html.includes('q-text-inherit'));assert.equal(JSON.parse(doc.querySelector('script[type="application/ld+json"]').textContent).itemListElement[0].author.name,'Name "quoted"');
 ...
 console.log('PASS CTA Reviews partial hardening: actual Liquid four layouts/presets, literal anchor and CSS scope, explicit false, two instances, persistent motion controls, manual review media, reduced motion, visibility, editor selection/unload/reload, safe JSON and neutral insertion defaults. Legacy provider widget contracts remain NOT REVIEWED.');
