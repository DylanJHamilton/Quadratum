...
(async()=>{
 ...
 const customDom=new JSDOM(await render('custom',{},false,[custom]));assert(customDom.window.document.querySelector('.qw3-icon-mask').getAttribute('style').includes('/assets/custom-logo.svg'));customDom.window.close();
 ...
 const legacy=new JSDOM(await render('legacy',{},false,[{...custom,settings:{...custom.settings,icon_custom_path:'icons/tabler/filled/brand-coinbase.svg'}}]));assert(legacy.window.document.querySelector('use').getAttribute('href').endsWith('#tabler-brand-coinbase'));legacy.window.close();
 console.log('PASS CTA Web3: actual preset and six layout combinations, configured links, editor-only simulation, absent action guidance, deployed symbols/custom asset, missing-icon text fallback, duplicate init/two instances, reveal fallback/reduced motion and editor cleanup. Legacy nested library paths resolve through the deployed sprite; custom flat assets retain mask tint. Live merchant/editor acceptance remains open.');
