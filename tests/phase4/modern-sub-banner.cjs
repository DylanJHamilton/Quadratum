...
const source = fs.readFileSync('sections/sub-banner-modern.liquid', 'utf8'), start = source.lastIndexOf('{% schema %}');
...
(async () => {
  for (const overlay of ['none', 'solid', 'gradient']) for (const align of ['left', 'center', 'right']) {
    ...
    const dom = new JSDOM(html), roots = [...dom.window.document.querySelectorAll('[data-modern-sub-banner]')];
    ...
    assert.equal(roots[0].querySelector('.q-content').style.textAlign, align); assert.equal(roots[0].style.getPropertyValue('--glass-radius'), '0px'); assert.equal(roots[0].style.getPropertyValue('--glass-blur'), '0px'); assert.equal(roots[0].style.getPropertyValue('--q-safe-offset'), '80px');
  ...
  console.log('PASS Modern Sub Banner: actual preset/two instances, overlay/alignment modes, literal anchors/scoped CSS, zero panel/button values, heading level, empty-link guard and color overrides. Responsive and Shopify acceptance remain open.');
