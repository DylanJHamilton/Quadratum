...
(async()=>{
  ...
  d=await fixture({settings:{destination:'shopify_customer'},attrs:'data-captcha="recaptcha_v3" data-captcha-key="key" data-q-pack-tags="0"',blocks:[f.field('email','email',{default_value:'native@example.test'}),f.field('utm','hidden',{name_attr:'contact[utm_source]',default_value:'merchant'})],url:'https://shop.test/fr/contact'});w=d.window;let executions=0;w.grecaptcha={ready(cb){cb()},execute(){executions++;return Promise.resolve('native-must-not-call')}};f.boot(w,assets);form=w.document.querySelector('form');form.querySelector('[name="contact[marketing_confirmation]"]').checked=true;assert(!submit(w,form).defaultPrevented);assert.equal(executions,0);assert.equal(form.querySelector('[name="contact[utm_source]"]').value,'merchant');w.close();
  d=await fixture({attrs:'data-captcha="turnstile"',blocks:[f.field('email','email',{name_attr:'email',default_value:'yes@example.test'}),f.field('metadata','hidden',{name_attr:'form_type',default_value:'customer'})]});w=d.window;f.boot(w,assets);form=w.document.querySelector('form');assert(submit(w,form).defaultPrevented);assert.match(form.querySelector('.q-live').textContent,/Verification is unavailable/);w.close();
