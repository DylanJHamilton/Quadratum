...
async function html(name, settings = {}, id = 'one', design = false, data = {}) {
  ...
  const output = await f.engine.parseAndRender(f.platform ? f.platform(liquid) : liquid, { block }, { globals: { request: { design_mode: design }, shop: { name: 'Fixture store' }, product: f.product, ...data, routes: { all_products_collection_url: '/fr/collections/all' } } });
