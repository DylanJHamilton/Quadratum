...
(() => {
  ...
  const selector = '[data-product-sku], [data-product-description]';
      ...
      const handle = event => {
        ...
        if (!sourceScope) {
          if (detail.sectionId != null) {
            ...
            const scopes = new Set([...document.querySelectorAll('[data-product-sku]')].filter(node => node.dataset.productId === root.dataset.productId).map(scopeOf));
