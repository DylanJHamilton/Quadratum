...
| GA4, Meta, TikTok ID fields | Deprecated; no loader, event subscription or tracking is installed. Values and IDs retained. | Configure provider apps or Shopify pixels. |
| `marketing_consent_mode` | Retained metadata sent by supported form hosts. Option values unchanged, labels now say metadata. No UI/consent mutation. Unused checked/label controls deprecated. | Native newsletter confirmation remains explicit; external handlers must independently process lawful opt-in. Shopify privacy settings/API own tracking permission. |
