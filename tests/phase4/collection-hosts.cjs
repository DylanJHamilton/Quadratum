...
(async()=>{
 ...
 for(const name of ['product-card','collection-simple-product-card','collection-modern-product-card']){const p={...product,variants:[{...v,quantity_rule:{min:2,increment:2,max:8}}],selected_or_first_available_variant:{...v,quantity_rule:{min:2,increment:2,max:8}}};d=doc(await snippet(name,{product:p,s:{show_quick_add:true,enable_quick_add:true,show_buy_now:true},enable_quick_add:true,enable_quick_view:true,show_action_row:true}));assert.equal(d.querySelectorAll('form').length,0,'minimum-quantity products select quantities on PDP');assert(!d.querySelector('[data-quick-view]'));d.defaultView.close();}
 ...
 d=doc(await render('collection-modern-quick-view',{}, {product:{...product,variants:[{...v,quantity_rule:{min:2}}]}}));assert.equal(d.querySelectorAll('form').length,0);assert(d.querySelector('.qcmQv__btn--view').textContent.includes('Choose options'));d.defaultView.close();
 ...
 const nativeQuickViewHTML=await render('collection-modern-quick-view',{}, {product:{...product,variants:[v,variant(22,true,2500)]}});qvButton.click();lr[1].resolve({ok:true,text:async()=>nativeQuickViewHTML});await drain();await new Promise(resolve=>setImmediate(resolve));assert(!qvHost.hidden);const variantSelect=qvHost.querySelector('select');assert(variantSelect);variantSelect.value='22';variantSelect.dispatchEvent(new lw.Event('change',{bubbles:true}));assert.equal(qvHost.querySelector('.qcmQv__priceNow').textContent,'$25.00');assert(qvHost.querySelector('.qcmQv__priceWas').hidden);assert.equal(new lw.FormData(variantSelect.form).get('id'),'22');
