...
<div class="qtmAccountRegister__passwordField"><input type="password" data-qtm-account-register-password-input><button data-qtm-account-register-password-toggle data-show-label="Show password" data-hide-label="Hide password"><span data-qtm-account-register-password-toggle-text>Show</span></button></div>`, {url:'https://example.test/fr/products/test',runScripts:'outside-only'});
